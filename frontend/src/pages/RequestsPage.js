  ...
  const handleCreate = async (e) => {
    ...
    try {
      const res = await requestsAPI.create({
        ...form,
        credit_amount: parseInt(form.credit_amount, 10)
      });
      ...
      {loading ? (
        ...
        <div style={s.list}>
          ...
          {recommendations.length > 0 && (
            <div style={s.card}>
              <div style={{ width: '100%' }}>
                <h3 style={s.cardTitle}>Recommended Providers</h3>
                <p style={s.cardMeta}>Based on your request</p>
                <ul style={{ margin: 0, paddingLeft: 18 }}>
                  {recommendations.map((rec, idx) => (
                    <li key={rec.id || idx} style={s.cardDesc}>
                      {rec.full_name || rec.name || `Recommendation ${idx + 1}`}
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <form onSubmit={handleCreate} style={s.form}>
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={e => setForm({ ...form, skill_required: e.target.value })}
                />
              </div>
              ...
              {['rating', 'professionalism', 'timeliness', 'quality'].map(field => (
                <div key={field} className="form-group">
                  ...
                  <input
                    ...
                    value={review[field]}
                    onChange={e =>
                      setReview({ ...review, [field]: parseInt(e.target.value, 10) })
                    }
                  />
                </div>
              ))}
